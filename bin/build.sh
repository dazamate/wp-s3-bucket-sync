...
# irrelevant. Output: build/daz-s3-image-sync/ and build/daz-s3-image-sync.zip
#
set -euo pipefail
...
# composer.json carries the PSR-4 map for our (un-prefixed) source. It is removed
# from the dist afterwards so a later `composer install` on the target site can't
# overwrite the scoped vendor/.
